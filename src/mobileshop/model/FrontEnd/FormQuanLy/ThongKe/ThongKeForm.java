package mobileshop.model.FrontEnd.FormQuanLy.ThongKe;

import com.github.lgooddatepicker.components.DatePicker;
import com.github.lgooddatepicker.components.DatePickerSettings;
import mobileshop.model.BackEnd.PriceFormatter;
import mobileshop.model.BackEnd.QuanLyChiTietHoaDon.ChiTietHoaDon;
import mobileshop.model.BackEnd.QuanLyChiTietHoaDon.QuanLyChiTietHoaDonBUS;
import mobileshop.model.BackEnd.QuanLyChiTietPN.ChiTietPhieuNhap;
import mobileshop.model.BackEnd.QuanLyChiTietPN.QuanLyChiTietPhieuNhapBUS;
import mobileshop.model.BackEnd.QuanLyHoaDon.HoaDon;
import mobileshop.model.BackEnd.QuanLyHoaDon.QuanLyHoaDonBUS;
import mobileshop.model.BackEnd.QuanLyKhachHang.KhachHang;
import mobileshop.model.BackEnd.QuanLyKhachHang.QuanLyKhachHangBUS;
import mobileshop.model.BackEnd.QuanLyNCC.NhaCungCap;
import mobileshop.model.BackEnd.QuanLyNCC.QuanLyNhaCungCapBUS;
import mobileshop.model.BackEnd.QuanLyNhanVien.NhanVien;
import mobileshop.model.BackEnd.QuanLyNhanVien.QuanLyNhanVienBUS;
import mobileshop.model.BackEnd.QuanLyPhieuNhap.PhieuNhap;
...
import mobileshop.model.BackEnd.QuanLySanPham.SanPham;
import mobileshop.model.FrontEnd.FormChon.ChonKhachHangForm;
import mobileshop.model.FrontEnd.FormChon.ChonNhaCungCapForm;
import mobileshop.model.FrontEnd.FormChon.ChonNhanVienForm;
import mobileshop.model.FrontEnd.FormChon.ChonSanPhamForm;
...
import mobileshop.model.FrontEnd.MyButton.RefreshButton;
...
public class ThongKeForm extends JPanel {
    ...
    public ThongKeForm() {
        this.setBackground(new Color(244, 242, 242));
...
//        tabs.addTab("Thống kê ver2", getIcon("icons8_company_30px.png"), tk2);
        ...
        tabs.addTab("Sản phẩm", getIcon("mobile_30px.png"), null);
        tabs.addTab("Nhân viên", getIcon("staff_30px.png"), null);
        tabs.addTab("Khách hàng", getIcon("user_30px.png"), null);
        tabs.addTab("Nhà cung cấp", getIcon("company_30px.png"), null);
    ...
    private ImageIcon getIcon(String filename) {
        return new ImageIcon(getClass().getResource("/mobileshop/images/" + filename));
    }
}
...
class ThongKe_Hoang extends JPanel {
    ...
    JPanel plBieuDo = new JPanel();
    JPanel plBieuDoCot = new JPanel();
    ...
    private void setDataToPanelTong() {
        ...
        plThongKeSoLuong.add(getJPanelTong("SẢN PHẨM", "mobile_shopping_100px.png", qlspBUS.getDssp().size(), Color.BLUE));
        plThongKeSoLuong.add(getJPanelTong("NHÂN VIÊN", "staff_100px.png", qlnvBUS.getDsnv().size(), Color.BLUE));
        plThongKeSoLuong.add(getJPanelTong("KHÁCH HÀNG", "user_100px.png", qlkhBUS.getDskh().size(), Color.BLUE));
        plThongKeSoLuong.add(getJPanelTong("NHÀ CUNG CẤP", "skyscrapers_100px.png", qlnccBUS.getDsncc().size(), Color.BLUE));
    }
    ...
    public void showPieChart(){
        ...
        barDataset.setValue( "Sản phẩm" , qlspBUS.getDssp().size() );  
        barDataset.setValue( "Nhân viên" , qlnvBUS.getDsnv().size() );   
        barDataset.setValue( "Khách hàng" , qlkhBUS.getDskh().size() );    
        barDataset.setValue( "Nhà Cung cấp" , qlnccBUS.getDsncc().size() );  
      
      //create chart
        JFreeChart piechart = ChartFactory.createPieChart("",barDataset, true,true,true);//explain
      
        PiePlot piePlot =(PiePlot) piechart.getPlot();
      
       //changing pie chart blocks colors
        piePlot.setSectionPaint("Sản phẩm", new Color(255,255,102));
        piePlot.setSectionPaint("Nhân viên", new Color(102,255,102));
        piePlot.setSectionPaint("Khách hàng", new Color(255,102,153));
        piePlot.setSectionPaint("Nhà cung cấp", new Color(0,204,204));
        ...
        //plB.removeAll();
        plThongKeSoLuong.add(barChartPanel, BorderLayout.WEST);
        plThongKeSoLuong.validate();
        
    }
    
        public void showBarChart(){
        DefaultCategoryDataset dataset = new DefaultCategoryDataset();
        dataset.setValue(qlspBUS.getDssp().size(), "Sản Phẩm", "Sản Phẩm");
        dataset.setValue(qlnvBUS.getDsnv().size(), "Nhân Viên", "Nhân Viên");
        dataset.setValue(qlkhBUS.getDskh().size(), "Khách Hàng", "Khách Hàng");
        dataset.setValue(qlnccBUS.getDsncc().size(), "Nhà Cung Cấp", "Nhà Cung Cấp");

        
        JFreeChart chart = ChartFactory.createBarChart("","Tổng số lượng","", 
        ...
        Color clr3 = new Color(208, 9, 208);
        ...
        plThongKeSoLuong.add(barpChartPanel, BorderLayout.WEST);
        plThongKeSoLuong.validate();
